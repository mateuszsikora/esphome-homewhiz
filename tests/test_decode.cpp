...
int main() {
  ...
  // --- spin is a sparse enum {no_spin, rinse_hold} in the config; byte 38 == 12
  // is NOT an enumerated value, so the decoder falls back to publishing the raw
  // value ("12") rather than leaving the entity stale. ---
  CHECK(decode_key(f, len, "WASHER_SPIN", v) && v.type == DECODED_TEXT &&
            std::strcmp(v.text, "12") == 0,
        "WASHER_SPIN(12) falls back to raw value \"12\"");
